...
export async function PUT(req: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    await connectDB();
    const user = await User.findByIdAndUpdate(params.id, parsed.data, { new: true }).select('-passwordHash');
    if (!user) return apiError('User not found', 404);

    audit({ actorId: session.user.id, action: 'UPDATE_USER', resource: 'User', resourceId: params.id });
...
export async function DELETE(req: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    await connectDB();
    const user = await User.findByIdAndUpdate(params.id, { isActive: false }, { new: true }).select('-passwordHash');
    if (!user) return apiError('User not found', 404);

    audit({ actorId: session.user.id, action: 'DEACTIVATE_USER', resource: 'User', resourceId: params.id });
